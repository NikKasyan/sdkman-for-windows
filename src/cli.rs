...
const COMMAND_GUIDE: &str = "Command guide:
  ...
  list       Show candidates, or versions for one candidate.
  install    Download an SDK version or register an existing local SDK.
  uninstall  Remove a downloaded SDK or unregister a local SDK. Alias: rm.
  use        Select a version for the current shell only.
  default    Set the default version and regenerate command shims.
  current    Show the active SDK version or all active versions.
  home       Print the active or version-specific SDK home.
