...
| `sdk default <candidate> [version]` | Sets the default version by updating the candidate `current` link, regenerating shims, and updating that candidate's HOME variables in the current shell. Shell initialization restores those HOME variables from the default. Omitted versions and ambiguous prefixes open an installed-version picker. | You want commands such as `java` or `mvn` to resolve to this version by default. |
